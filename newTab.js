...
async function sendMessage() {
    ...
    const sendAeTlMsg = (arg = "normalDevices") => {
        let finalizeData = null;

        if (arg === "normalDevices") {
            finalizeData = TLDevice.filter(device => device.Branch !== "NSAH").sort((a, b) => {
                if (a.Branch === "SBLR") return -1;
                if (b.Branch === "SBLR") return 1;
                return 0;
            });
        } else {
            finalizeData = TLDevice.filter(device => device.Branch === "NSAH");
        }

        for (let i = 0; i < finalizeData.length; i++) {
            const x = finalizeData[i];
    ...
    sendAeTlMsg();
    cron.schedule('7 15 * * *', () => {
        console.log('Running a cron at 12:01 PM IST');
        sendAeTlMsg('NSAHDevices');
    });
