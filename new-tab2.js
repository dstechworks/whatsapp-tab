...
async function sendMessage() {
    ...
    function mergeAllData() {
        let temp = [];
        // Devices within the last hour are marked as 'Active' and others as 'InActive'
        const withInFiveMin = new Date(new Date().getTime() - 5 * 60 * 1000);;
        const lastHr = new Date(new Date().getTime() - 60 * 60 * 1000);
        findDevices('getMatchedDevices').forEach(x => {
            if (x?.updated_timestamp && x?.branch != null) {
                if (new Date(x.updated_timestamp) > withInFiveMin) {
                    x['Status'] = 'Active';
                }
                if (new Date(x.updated_timestamp) < withInFiveMin) {
                    x['Status'] = 'InActive';
                }
                if (new Date(x.updated_timestamp) > lastHr) {
                    x['LastHrStatus'] = 'Active';
                }
                if (new Date(x.updated_timestamp) < lastHr) {
                    x['LastHrStatus'] = 'InActive';
                }
